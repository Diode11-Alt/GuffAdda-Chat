...
class ConversationService {
    ...
    static async findDirectConversation(userId1, userId2) {
      ...
      LIMIT 1
    `;
        const res = await (0, db_1.query)(sql, [userId1, userId2]);
        return res.rows[0]?.id || null;
    }
    /**
     * Create a new conversation (direct or group).
     */
    static async create(createdBy, type, memberIds, name) {
        ...
        const convRes = await (0, db_1.query)(`INSERT INTO conversations (type, name, created_by, updated_at)
       VALUES ($1, $2, $3, NOW())
       RETURNING *`, [type, name || null, createdBy]);
        const conversation = convRes.rows[0];
        // Add creator as admin
        await (0, db_1.query)(`INSERT INTO conversation_members (conversation_id, user_id, role, unread_count)
       VALUES ($1, $2, 'admin', 0)`, [conversation.id, createdBy]);
        // Add other members
        for (const memberId of memberIds) {
            if (memberId !== createdBy) {
                await (0, db_1.query)(`INSERT INTO conversation_members (conversation_id, user_id, role, unread_count)
           VALUES ($1, $2, 'member', 0)
           ON CONFLICT DO NOTHING`, [conversation.id, memberId]);
            }
        }
     ...
     */
    static async getById(conversationId, requestingUserId) {
        const convRes = await (0, db_1.query)('SELECT * FROM conversations WHERE id = $1', [conversationId]);
        if (convRes.rows.length === 0)
            return null;
        const membersRes = await (0, db_1.query)(`SELECT cm.role, cm.unread_count, cm.joined_at,
       ...
       WHERE cm.conversation_id = $1`, [conversationId]);
        ...
        if (conversation.type === 'direct') {
            const otherUser = members.find((m) => m.id !== requestingUserId);
            if (otherUser) {
    ...
    static async isMember(conversationId, userId) {
        const res = await (0, db_1.query)('SELECT 1 FROM conversation_members WHERE conversation_id = $1 AND user_id = $2', [conversationId, userId]);
    ...
    static async updateLastMessage(conversationId, messageId) {
        await (0, db_1.query)(`UPDATE conversations SET last_message_id = $1, last_message_at = NOW(), updated_at = NOW() WHERE id = $2`, [messageId, conversationId]);
    }
    /**
     * Increment unread count for all members except the sender.
     */
    static async incrementUnread(conversationId, excludeUserId) {
        await (0, db_1.query)(`UPDATE conversation_members SET unread_count = unread_count + 1
       WHERE conversation_id = $1 AND user_id != $2`, [conversationId, excludeUserId]);
    }
    /**
     * Reset unread count for a user in a conversation.
     */
    static async markAsRead(conversationId, userId) {
        await (0, db_1.query)(`UPDATE conversation_members SET unread_count = 0 WHERE conversation_id = $1 AND user_id = $2`, [conversationId, userId]);
    }
    /**
     * Update group info.
     */
    static async updateGroupInfo(conversationId, name, avatarUrl) {
    ...
    static async removeMember(conversationId, userId) {
        await (0, db_1.query)(`DELETE FROM conversation_members WHERE conversation_id = $1 AND user_id = $2`, [conversationId, userId]);
    }
}
