...
export class MessageService {
  ...
  static async create(data: {
    ...
    const res = await query(
      `INSERT INTO messages (conversation_id, sender_id, message_type, encrypted_payload, iv, reply_to_id)
       VALUES ($1, $2, $3, $4, $5, $6)
       RETURNING *`,
      [
        data.conversationId,
   ...
   */
  static async getHistory(conversationId: string, userId: string, options: { before?: string; limit?: number }) {
    ...
    const messages = res.rows.map(row => {
      let status = 'sent';
      if (row.read_level === 2) status = 'read';
      else if (row.read_level === 1) status = 'delivered';
      return { ...row, status };
    }).reverse();
   ...
   */
  static async updateStatus(messageId: string, userId: string, status: 'delivered' | 'read') {
    await query(
  ...
  static async deleteMessage(messageId: string, userId: string, forEveryone: boolean) {
    if (forEveryone) {
      ...
      await query(
        `UPDATE messages SET is_deleted = true, deleted_for_everyone_at = NOW()
  ...
  static async editMessage(messageId: string, userId: string, newContent: string) {
    const res = await query(
      `UPDATE messages SET encrypted_payload = $1, is_edited = true, updated_at = NOW()
   ...
   */
  static async setPinned(messageId: string, isPinned: boolean) {
    const res = await query(
   ...
   */
  static async search(userId: string, queryText: string, limit = 20) {
    const res = await query(
      `
   ...
   */
  static async removeReaction(messageId: string, userId: string, reaction: string) {
    await query(
